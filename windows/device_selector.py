...
class DeviceSelector:
    ...
    def setup_ui(self):
        padding_main = scale(40)

        main_frame = ttk.Frame(self.root, padding=padding_main)
        main_frame.pack(fill=tk.BOTH, expand=True)

        title_font = ("Arial", scale_font_size(14))
        label_font = ("Arial", scale_font_size(12))
        status_font = ("Arial", scale_font_size(10))
        info_font = ("Arial", scale_font_size(10))
        button_font = ("Arial", scale_font_size(12))

        title = ttk.Label(main_frame,
                          text="Чтобы начать мониторинг,\nвыберите устройство, с которого хотите получать данные",
                          font=title_font, justify='center')
        title.pack(pady=(0, scale(30)))

        select_frame = ttk.Frame(main_frame)
        select_frame.pack(pady=scale(10))

        ttk.Label(select_frame, text="Выбранное устройство:", font=label_font).pack(side='left', padx=scale(10))

        self.device_combo = ttk.Combobox(select_frame, font=label_font,
                                         width=scale(25), state='readonly')
        self.device_combo.pack(side='left', padx=scale(10))
        self.device_combo.set("не выбрано")

        self.status_var = tk.StringVar(value="Загрузка списка устройств...")
        status_label = ttk.Label(main_frame, textvariable=self.status_var,
                                 font=status_font, foreground="gray")
        status_label.pack(pady=scale(15))
        ...
        info_label = ttk.Label(main_frame, text=info_text, font=info_font,
                               foreground="gray", justify='center')
        info_label.pack(pady=scale(20))

        btn_frame = ttk.Frame(main_frame)
        btn_frame.pack(pady=scale(20))

        self.next_btn = ttk.Button(btn_frame, text="ДАЛЕЕ", command=self.start_monitoring,
                                   state='disabled', style='Action.TButton', width=scale(15))
        self.next_btn.pack(side='left', padx=scale(10))

        back_btn = ttk.Button(btn_frame, text="НАЗАД", command=self.back_to_welcome,
                              style='Action.TButton', width=scale(15))
        back_btn.pack(side='left', padx=scale(10))
