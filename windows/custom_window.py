...
class CustomWindow:
    ...
    def setup_ui(self):
        ...
        ttk.Separator(self.scrollable_frame, orient='horizontal').grid(row=1, column=0, columnspan=2,
                                                                       sticky='ew', pady=scale(5))

        self.setup_current_data(label_font, bold_font)
        self.setup_statistics(label_font, bold_font)
        self.setup_compliance(label_font, bold_font)
    ...
    def setup_compliance(self, label_font, bold_font):
        comp_frame = ttk.LabelFrame(self.scrollable_frame, text="ОЦЕНКА СООТВЕТСТВИЯ НОРМАМ",
                                    padding=scale(10))
        comp_frame.grid(row=4, column=0, columnspan=2, sticky='ew', padx=scale(10), pady=scale(5))
        ...
        ttk.Label(comp_frame, text="Температура:", font=label_font).grid(row=0, column=0, sticky='w')
        ttk.Label(comp_frame, textvariable=self.temp_comp, font=bold_font).grid(row=0, column=1, sticky='w')

        ttk.Label(comp_frame, text="Влажность:", font=label_font).grid(row=1, column=0, sticky='w')
        ttk.Label(comp_frame, textvariable=self.hum_comp, font=bold_font).grid(row=1, column=1, sticky='w')

        ttk.Label(comp_frame, text="Давление:", font=label_font).grid(row=2, column=0, sticky='w')
        ttk.Label(comp_frame, textvariable=self.press_comp, font=bold_font).grid(row=2, column=1, sticky='w')

    def setup_trends(self, label_font, bold_font):
        trend_frame = ttk.LabelFrame(self.scrollable_frame, text="ТРЕНДЫ (за 3 часа)",
                                     padding=scale(10))
        trend_frame.grid(row=5, column=0, columnspan=2, sticky='ew', padx=scale(10), pady=scale(5))
        ...
        ttk.Label(trend_frame, text="Температура:", font=label_font).grid(row=0, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_t, font=bold_font).grid(row=0, column=1, sticky='w')

        ttk.Label(trend_frame, text="Влажность:", font=label_font).grid(row=1, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_h, font=bold_font).grid(row=1, column=1, sticky='w')

        ttk.Label(trend_frame, text="Давление:", font=label_font).grid(row=2, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_p, font=bold_font).grid(row=2, column=1, sticky='w')

    def setup_events(self, label_font, italic_font):
        events_frame = ttk.LabelFrame(self.scrollable_frame, text="НАБЛЮДАЕМЫЕ ЯВЛЕНИЯ",
                                      padding=scale(10))
        events_frame.grid(row=6, column=0, columnspan=2, sticky='ew', padx=scale(10), pady=scale(5))
    ...
    def setup_recommendations(self, label_font, italic_font):
        rec_frame = ttk.LabelFrame(self.scrollable_frame, text="РЕКОМЕНДАЦИИ",
                                   padding=scale(10))
        rec_frame.grid(row=7, column=0, columnspan=2, sticky='ew', padx=scale(10), pady=scale(5))
    ...
    def if_temp_critical(self) -> bool:
        if not self.records or self.min_temp is None or self.max_temp is None or self.crit_percent_temp is None:
            return False
        return self.check_critical(self.records[-1]['temperature'], self.min_temp, self.max_temp,
                                   self.crit_percent_temp)
    ...
    def if_press_critical(self) -> bool:
        if not self.records or self.min_press is None or self.max_press is None or self.crit_percent_press is None:
            return False
        return self.check_critical(self.records[-1]['pressure'], self.min_press, self.max_press,
                                   self.crit_percent_press)
    ...
    def update_analysis(self):
        ...
        if events:
            for event in events:
                ...
                for category in self.rec_data.values():
                    if event in category:
                        ...
                        if icon:
                            ttk.Label(frame, text=icon, font=emoji_font).pack(side='left', padx=(0, scale(10)))
                        ttk.Label(frame, text=text, wraplength=scale(580), justify='left').pack(side='left',
                                                                                                expand=True, fill='x')
                        found = True
                        break
                if not found:
            ...
            ttk.Label(self.rec_container, text="Рекомендаций нет",
                      font=("Arial", scale_font_size(12), "italic")).pack(pady=scale(10))
