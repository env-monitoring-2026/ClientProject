...
class ConfigWindow:
    ...
    def setup_ui(self):
        ...
        sms_frame = ttk.LabelFrame(main_frame, text="КОНФИГУРАЦИЯ SMS", padding=15)
        sms_frame.grid(row=2, column=0, columnspan=2, sticky='ew', pady=10)

        ttk.Label(sms_frame, text="Номер телефона:", font=("Arial", 11)).grid(row=0, column=0, sticky='w', pady=5)
        phone_entry = ttk.Entry(sms_frame, textvariable=self.phone_var, width=20, font=("Arial", 11))
        phone_entry.grid(row=0, column=1, sticky='w', pady=5, padx=10)
        ttk.Label(sms_frame, text="+7XXXXXXXXXX (только цифры)", font=("Arial", 9, "italic")).grid(row=1, column=1,
                                                                                                   sticky='w')

        ttk.Checkbutton(sms_frame, text="Отправлять SMS-сообщения о параметрах окружающей среды",
                        variable=self.send_params_var).grid(row=2, column=0, columnspan=2, sticky='w', pady=10)

        ttk.Label(sms_frame, text="Частота отправки:", font=("Arial", 11)).grid(row=3, column=0, sticky='w', pady=5)
        ...
        for i, (text, value) in enumerate(frequencies):
            ...
            rb.grid(row=i // 3, column=i % 3, sticky='w', padx=10, pady=2)

        ttk.Checkbutton(sms_frame, text="Отправлять диагностические SMS-сообщения",
                        variable=self.send_diagnostic_var).grid(row=4, column=0, columnspan=2, sticky='w', pady=10)

        ttk.Separator(main_frame, orient='horizontal').grid(row=3, column=0, columnspan=2, sticky='ew', pady=15)

        ftp_frame = ttk.LabelFrame(main_frame, text="КОНФИГУРАЦИЯ ГЛАВНОГО ОКНА", padding=15)
        ftp_frame.grid(row=4, column=0, columnspan=2, sticky='ew', pady=10)

        ttk.Label(ftp_frame, text="Рабочая директория:", font=("Arial", 11)).grid(row=0, column=0, sticky='w', pady=5)

        dir_frame = ttk.Frame(ftp_frame)
        dir_frame.grid(row=0, column=1, sticky='w', pady=5, padx=10)

        dir_entry = ttk.Entry(dir_frame, textvariable=self.work_dir_var, width=40, font=("Arial", 10))
        ...
        ttk.Label(ftp_frame, text="Сюда будут сохраняться графики и файл data.txt",
                  font=("Arial", 9, "italic")).grid(row=1, column=1, sticky='w')

        ttk.Label(ftp_frame, text="Время сброса окна:", font=("Arial", 11)).grid(row=2, column=0, sticky='w', pady=10)

        reset_frame = ttk.Frame(ftp_frame)
        reset_frame.grid(row=2, column=1, sticky='w', pady=10)
        ...
        warning_label = ttk.Label(warning_frame, text=warning_text,
                                  font=("Arial", 10, "bold"), foreground="red", justify='center')
        warning_label.pack()
