...
class IndoorWindow:
    ...
    def setup_ui(self):
        ...
        self.setup_microclimate(label_font, bold_font)
        ...
        update_label = ttk.Label(self.scrollable_frame, textvariable=self.update_time_var,
                                 font=small_font)
        update_label.grid(row=21, column=0, columnspan=2, pady=scale(5))
    ...
    def setup_microclimate(self, label_font, bold_font):
        micro_frame = ttk.LabelFrame(self.scrollable_frame, text="ОЦЕНКА МИКРОКЛИМАТА",
                                     padding=scale(10))
        micro_frame.grid(row=4, column=0, columnspan=2, sticky='ew', padx=scale(10), pady=scale(5))
        ...
        ttk.Label(micro_frame, text="Температура:", font=label_font).grid(row=0, column=0, sticky='w')
        ttk.Label(micro_frame, textvariable=self.temp_rating, font=bold_font).grid(row=0, column=1, sticky='w')

        ttk.Label(micro_frame, text="Влажность:", font=label_font).grid(row=1, column=0, sticky='w')
        ttk.Label(micro_frame, textvariable=self.hum_rating, font=bold_font).grid(row=1, column=1, sticky='w')

        ttk.Label(micro_frame, text="Риск конденсации:", font=label_font).grid(row=2, column=0, sticky='w')
        ttk.Label(micro_frame, textvariable=self.cond_risk, font=bold_font).grid(row=2, column=1, sticky='w')

        ttk.Label(micro_frame, text="Риск плесени:", font=label_font).grid(row=3, column=0, sticky='w')
        ttk.Label(micro_frame, textvariable=self.mold_risk, font=bold_font).grid(row=3, column=1, sticky='w')
    ...
    def setup_events(self, label_font, italic_font):
        events_frame = ttk.LabelFrame(self.scrollable_frame, text="НАБЛЮДАЕМЫЕ ЯВЛЕНИЯ В ПОМЕЩЕНИИ",
                                      padding=scale(10))
    ...
    def update_analysis(self):
        if not self.records or len(self.records) < 2:
            ...
            for widget in self.events_container.winfo_children():
                widget.destroy()
            ttk.Label(self.events_container, text="Нет данных для анализа",
