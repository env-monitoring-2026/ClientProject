...
class StreetWindow:
    ...
    def setup_ui(self):
        ...
        self.setup_weather_character()

        self.setup_precipitation()
    ...
    def setup_weather_character(self):
        char_frame = ttk.LabelFrame(self.scrollable_frame, text="ХАРАКТЕР ВОЗДУШНЫХ МАСС", padding=10)
        char_frame.grid(row=4, column=0, columnspan=2, sticky='ew', padx=10, pady=5)
        char_frame.columnconfigure(1, weight=1)

        self.weather_type = tk.StringVar(value="--")
        ttk.Label(char_frame, text="Тип:", font=("Arial", 11)).grid(row=0, column=0, sticky='w')
        ttk.Label(char_frame, textvariable=self.weather_type, font=("Arial", 11, "bold")).grid(row=0, column=1, sticky='w')

    def setup_precipitation(self):
        precip_frame = ttk.LabelFrame(self.scrollable_frame, text="ОСАДКИ", padding=10)
        precip_frame.grid(row=5, column=0, columnspan=2, sticky='ew', padx=10, pady=5)

        self.precip_type = tk.StringVar(value="--")
        self.precip_prob = tk.StringVar(value="--")

        ttk.Label(precip_frame, text="Тип:", font=("Arial", 11)).grid(row=0, column=0, sticky='w')
        ttk.Label(precip_frame, textvariable=self.precip_type, font=("Arial", 11, "bold")).grid(row=0, column=1, sticky='w')

        ttk.Label(precip_frame, text="Вероятность:", font=("Arial", 11)).grid(row=1, column=0, sticky='w')
        ttk.Label(precip_frame, textvariable=self.precip_prob, font=("Arial", 11, "bold")).grid(row=1, column=1, sticky='w')

    def setup_trends(self):
        trend_frame = ttk.LabelFrame(self.scrollable_frame, text="ТРЕНДЫ (за 3 часа)", padding=10)
        trend_frame.grid(row=6, column=0, columnspan=2, sticky='ew', padx=10, pady=5)
        ...
        ttk.Label(trend_frame, text="Давление:", font=("Arial", 11)).grid(row=0, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_p, font=("Arial", 11, "bold")).grid(row=0, column=1, sticky='w')

        ttk.Label(trend_frame, text="Температура:", font=("Arial", 11)).grid(row=1, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_t, font=("Arial", 11, "bold")).grid(row=1, column=1, sticky='w')

        ttk.Label(trend_frame, text="Влажность:", font=("Arial", 11)).grid(row=2, column=0, sticky='w')
        ttk.Label(trend_frame, textvariable=self.trend_h, font=("Arial", 11, "bold")).grid(row=2, column=1, sticky='w')

    def setup_events(self):
        events_frame = ttk.LabelFrame(self.scrollable_frame, text="НАБЛЮДАЕМЫЕ ПОГОДНЫЕ ЯВЛЕНИЯ", padding=10)
        events_frame.grid(row=7, column=0, columnspan=2, sticky='ew', padx=10, pady=5)
    ...
    def setup_recommendations(self):
        ...
        rec_frame.grid(row=8, column=0, columnspan=2, sticky='ew', padx=10, pady=5)
